...
_A cross-platform and performant React component that loads and renders images._

Available on platforms android, ios, tvos, web, expo-go
...
You can configure build-time settings for `expo-image` using its [config plugin](https://docs.expo.dev/config-plugins/introduction/). Add the plugin to the `plugins` array in your **app.json** or **app.config.js** and rebuild the native project.
...
<ConfigReactNative>
  If you are not using Continuous Native Generation ([CNG](https://docs.expo.dev/workflow/continuous-native-generation/))
